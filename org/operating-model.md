...
**Wealth Architect HQ logo — FINAL, canonical file saved.** PROTON built the final version himself in ChatGPT: a circular dark emblem/badge, gold compass+arrow icon on top, "THE WEALTH ARCHITECT" wordmark banner beneath it, gold ring border. Saved at `product/assets/logo/wealth-architect-hq-badge-final.png` — **this is now the primary logo file and the reference image every future generation must attach via `image_references`.** The badge/emblem format supersedes the side-by-side flat lockups PRISM generated (those didn't land) — see the lesson logged in `org/brand-standards.md`. `wealth-architect-hq-hero-gold.png` is historical reference only, not the shipping mark. **Still open per the Logos & Marks standard:** this file has glow/light-flare effects on the metal and ring, which the standard flags as an amateur/AI tell for a *flat vector master* — but PROTON has final say and has approved this exact file as the brand's shipping mark, so it ships as-is. A flat single-color version for embroidery/small-scale/single-color print use can be built later if a real use case needs it (e.g. a POD apparel print), not before.
